...
public class BadLinksService {
            ...
            boolean notificationSent =
                    notifyVendors(
                            ...
                            (monitoringRecord == null
                                    ? null
                                    : monitoringRecord.getLastEmailSentAt()),
                            scope);
            if (notificationSent) {
                monitoringDao.upsertLastEmailSentAt(building, Timestamp.from(now));
            }
        }
    }
    ...
    private boolean notifyVendors(
        ...
        if (lastEmailSentAt == null) {
            body.append("Links down detected")
                    .append("\n")
                    .append("Detected Time(UTC): ")
                    .append(GeneralUtils.formatTimestamp(Timestamp.from(now)))
                    .append("\n")
                    .append("Links Down Detected: ")
                    .append(rows.size())
                    .append("\n\n");
        } else {
            body.append("Additional links down detected since last notification.")
                    .append("\n")
                    .append("Last Email sent at(UTC): ")
                    .append(GeneralUtils.formatTimestamp(lastEmailSentAt))
                    .append("\n")
                    .append("Current Detected Time(UTC): ")
                    .append(GeneralUtils.formatTimestamp(Timestamp.from(now)))
                    .append("\n")
                    .append("Additional Links Down Detected: ")
                    .append(rows.size())
                    .append("\n\n");
        }
