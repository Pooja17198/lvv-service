...
class BadLinksResourceTest {
    ...
    @Mock LvvServiceApiConfiguration config;
    ...
    void testGetBadLinks_success() {
        BadLinksResource resource = new BadLinksResource(badLinksService, config);
        ...
        try (MockedStatic<MetricsScope> staticMock = mockStatic(MetricsScope.class)) {
            ...
            when(config.isEnableNetworkMonitoringAndAlerting()).thenReturn(true);
    ...
    @Test
    void testGetBadLinks_networkMonitoringDisabled_returnsEmptyList() {
        BadLinksResource resource = new BadLinksResource(badLinksService, config);
        String buildingName = "BLD-1";
        when(config.isEnableNetworkMonitoringAndAlerting()).thenReturn(false);
